...
my.packages <- c('plyr', 'tidyverse', 'data.table', 'anchors', 'textclean',
  'measurements', 'naniar','CoordinateCleaner','rnaturalearth','maps','sp',
  'spatialEco')
...
all_data_raw <- read.exsitu.csv(file.path(main_dir,"inputs",
  "raw_occurrence","exsitu_standard_column_names"), "2018")
...
# IF NEEDED: remove unused columns or rename columns
  all_data <- all_data[ , -which(names(all_data) %in%
    c("altitude_m","author","availability","datum","Diameter","elevation",
    "elevation_ft","family","garden_lat","garden_long","habit","order",
    "seed_zone","specific2","voucher"))]
...
all_data5$genus_new <- mgsub(all_data5$genus_new,
  c("=","Carrya","Fagusfagus","^Linder$","^Sasafras$","^Sassafrass$","Taxis"),
  c("","Carya","Fagus","Lindera","Sassafras","Sassafras","Taxus"))
...
all_data6$species_new <- gsub("americanaAAFuerteA","americana",all_data6$species_new)
all_data6$species_new <- gsub("flexIlis","flexilis",all_data6$species_new)
all_data6$species_new <- gsub("Illinoensis","illinoensis",all_data6$species_new)
all_data6$species_new <- gsub("IN","",all_data6$species_new)
all_data6$species_new <- gsub("PALUSTRIS","palustris",all_data6$species_new)
all_data6$species_new <- gsub("BORBONIA","borbonia",all_data6$species_new)
# remove remaining cultivars without species name
all_data7 <- all_data6 %>% filter(!grepl("[A-Z]",species_new))
nrow(all_data7) #52892
# see records removed:
...
# remove duplicate columns from taxon list
taxon_list <- taxon_list %>%
  dplyr::select(-genus,-species,-infra_rank,-infra_name)
...
all_data10 <- rbind(matched,need_match)
  table(all_data10$list) # desiderata: 27247 | synonym: 322
  # see how many rows have taxon name match
nrow(all_data10[which(!is.na(all_data10$list)),]) #27569
...
all_data10 <- all_data10 %>% dplyr::select(
  ...
  # locality
  orig_lat,orig_long,locality,municipality,county,state,country,habitat,
  # source
...
check <- all_data10 %>% filter(is.na(list))
...
#   needs to be preserved but is in wrong place
  all_data11$notes[grep("clearlake,california(38'n)",
    all_data11$prov_type)] <- "clearlake,california(38'n)"
...
all_data12 <- rbind(good,convert); nrow(all_data12) #50667
...
all_data12$locality[which(all_data12$locality == ";")] <- NA
all_data12$orig_source[which(all_data12$orig_source == ";")] <- NA
all_data12$orig_source[which(all_data12$orig_source == "#")] <- NA
all_data12$orig_source[which(all_data12$orig_source == "; ?")] <- NA
all_data12$orig_source <- gsub("unknown source","",all_data12$orig_source,
  ignore.case = T)
...
#all_data12[which(all_data12$UID == "id00001698"),]
...
all_data13 <- all_data12 %>% dplyr::select(
  ...
  orig_source,notes,habitat,num_indiv,acc_num,
  latlong_country,
  # source
  ...
  inst_name,inst_country,inst_continent,inst_lat,inst_long,inst_date_received)
str(all_data13)
...
all_data14 <- all_data12 %>%
  ...
  ungroup() %>%
  # remove duplicates
  distinct(all_locality,flag,gps_det,prov_type,
  inst_short,species_name_acc,.keep_all=T) %>%
  # reorder columns
  dplyr::select(
    species_name_acc,target_species,
    ## GeoLocate
    all_locality,locality,municipality,county,state,country,orig_source,
    latitude,longitude,
    ## record metadata
...
# write file
write.csv(all_data14, file.path(main_dir,"inputs",
  paste0("preGeolocate_exsitu_compiled_standardized_", Sys.Date(), ".csv")),
  row.names = F)
...
##
## RENAME COLUMNS FOR IN SITU DATA COMPILATION SCRIPT
##

# read in post-geolocate data
post_geo <- read.csv(file.path(main_dir,"inputs",
  "postGeolocate_exsitu_compiled_standardized_2020-11-12.csv"),
  fileEncoding="LATIN1",strip.white=TRUE,stringsAsFactors=F,
  na.strings=c("","NA"))

exsitu <- post_geo %>%
  dplyr::select(taxon_full_name,prov_type,sum_num_indiv,latitude,#coll_year,
    longitude,gps_det,locality,county,state,country,inst_short,#coord_precision,
    coll_num_CONCAT,UID_CONCAT,municipality,orig_source) %>%
  rename(taxon_name = taxon_full_name,
         #year = coll_year,
         basisOfRecord = prov_type,
         establishmentMeans = sum_num_indiv,
         ...
         publisher = coll_num_CONCAT,
         nativeDatabaseID = UID_CONCAT,
         references = orig_source)
exsitu$database <- "Ex_situ"

# write file
write.csv(exsitu, file.path(main_dir,"inputs","compiled_occurrence",
  "exsitu.csv"),row.names = F)
...
##
## ADD GEOLOCATED INFO TO OHER DATA ((did manual processing after this!!))
##


with_geo <- read.csv(file.path(main_dir,"inputs",
  "allData_postGeolocate_exsitu_compiled_standardized_2020-10-20-8.csv"),
  fileEncoding="LATIN1",strip.white=TRUE,colClasses="character",na.strings=c("","NA"))
add_geo <- with_geo %>%
  distinct(UID_CONCAT,.keep_all=T) %>%
  dplyr::select(latitude,longitude,gps_det,prov_type,UID_CONCAT)
needs_geo <- all_data14 %>%
  filter(UID_CONCAT %in% with_geo$UID_CONCAT) %>%
  dplyr::select(-latitude,-longitude,-gps_det,-prov_type)

mid <- join(type="left",needs_geo,add_geo)
add <- all_data14 %>% filter(!(UID_CONCAT %in% with_geo$UID_CONCAT))
all_data15 <- rbind(add,mid)

# write file
write.csv(all_data15, file.path(main_dir,"inputs",
  paste0("prelim_postGeolocate_exsitu_compiled_standardized_", Sys.Date(), ".csv")),
  row.names = F)
